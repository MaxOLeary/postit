...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    private func build(with data: NoteData) {
        ...
        // ink swatches — three primary-color dots; click to color the
        // selection / typing ink, re-click the active one for default white
        for (i, inkDef) in Style.inks.enumerated() {
            let dot = NSButton(frame: NSRect(x: 92 + CGFloat(i) * 17,
                                             y: (Style.stripHeight - 12) / 2,
                                             width: 12, height: 12))
            dot.title = ""

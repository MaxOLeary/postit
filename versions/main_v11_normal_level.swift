...
final class AppDelegate: NSObject, NSApplicationDelegate, NSTextViewDelegate, NSWindowDelegate {
    ...
    var glass: NSGlassEffectView!

    // Two tints: the calm look when idle, and a brighter one applied while
    // focused to counteract macOS dimming the glass on the active window.
    let idleTint    = NSColor(calibratedWhite: 0.16, alpha: 0.18)
    let focusedTint = NSColor(calibratedWhite: 0.85, alpha: 0.12)

    func applyTint(focused: Bool) {
        glass?.tintColor = focused ? focusedTint : idleTint
    }
    ...
    func windowDidResignKey(_ notification: Notification) { applyTint(focused: false) }

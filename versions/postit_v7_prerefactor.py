...
class GrowingTextEdit(QTextEdit):
    ...
    def __init__(self, text=""):
        ...
        self.setStyleSheet(
            ...
            f"selection-background-color:#3E7BD0;}}")
        self.setText(text)
...
class Dropdown(QWidget):
    ...
    def __init__(self, title="", body="", collapsed=False,
        ...
        # ---- collapsible body ----
        self.body = GrowingTextEdit(body)
        # indent the body a touch so it reads as "inside" the section
        self.body.setStyleSheet(self.body.styleSheet().replace(
            "QTextEdit{", "QTextEdit{padding-left:20px;"))
        if on_change:
...
class Postit(QWidget):
    ...
    def remove_block(self, widget):
        ...
        if self._active_block is widget:
            self._active_block = None
        # never leave the note with nothing to type in
        if not [b for b in self._blocks() if b is not widget]:
            self._add_text("")
        self.schedule_save()

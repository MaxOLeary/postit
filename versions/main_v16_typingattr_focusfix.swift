...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    private func build(with data: NoteData) {
        ...
        // font stepper (up/down = size up/down)
        stepper = NSStepper(frame: NSRect(x: 34, y: (Style.stripHeight - 19) / 2, width: 16, height: 19))
        stepper.minValue = Double(Style.minFont)
        stepper.maxValue = Double(Style.maxFont)
        stepper.increment = 1
        stepper.integerValue = Int(fontSize)
        stepper.valueWraps = false
        stepper.autoresizingMask = [.maxXMargin, .minYMargin]
        stepper.refusesFirstResponder = true       // keep the caret in the text
        stepper.target = self
        stepper.action = #selector(fontStepped)
        stepper.toolTip = "Font size"
    ...
    @objc private func fontStepped() {
        fontSize = CGFloat(stepper.integerValue)
        sizeLabel.stringValue = "\(Int(fontSize))"
    ...
    /// Keep the stepper + readout showing the font size at the cursor/selection,
    /// so the control reflects what you're actually editing.
    func textViewDidChangeSelection(_ notification: Notification) {
        guard let ts = textView.textStorage, ts.length > 0 else { return }
        let sel = textView.selectedRange()
        let idx = min(max(sel.location - (sel.location == ts.length ? 1 : 0), 0), ts.length - 1)
        if let f = ts.attribute(.font, at: idx, effectiveRange: nil) as? NSFont {
            let s = Int(round(f.pointSize))
            stepper.integerValue = s
            sizeLabel.stringValue = "\(s)"
        }
    }

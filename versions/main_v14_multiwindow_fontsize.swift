...
struct NoteData: Codable {
    ...
    init(id: String, text: String = "", fontSize: CGFloat = Style.defaultFont,
         frame: [CGFloat] = []) {
        self.id = id
        self.text = text
...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    private func snapshot() -> NoteData {
        let f = window.frame
        return NoteData(id: id, text: textView.string, fontSize: fontSize,
    ...
    @objc private func closeNote() { window.performClose(nil) }

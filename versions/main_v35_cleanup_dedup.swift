...
final class NoteStore {
    ...
    private static func noteFiles(sortedBy key: URLResourceKey, ascending: Bool) -> [URL] {
        ...
        func date(_ u: URL) -> Date {
            (try? u.resourceValues(forKeys: [key]))?.allValues[key] as? Date ?? .distantPast
        }
        return urls.filter { $0.pathExtension == "json" }
                   .sorted { ascending ? date($0) < date($1) : date($0) > date($1) }
    }
...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    @objc private func insertSection() {
        ...
        if let tv = activeText as? GrowingTextView,
           let idx = contentStack.arrangedSubviews.firstIndex(of: tv) {
            splitAndInsertSection(in: tv, at: idx)
            return
        }
        // Cursor is in a section body (or nowhere) → insert right after that block.
        insertFreshSection(at: min(activeBlockIndex() + 1, contentStack.arrangedSubviews.count))
    }
    ...
    private func insertFreshSection(at index: Int) {
    ...
    private func inkButton(at origin: NSPoint, action: Selector) -> NSButton {
        let b = NSButton(frame: NSRect(origin: origin, size: NSSize(width: 12, height: 12)))
        b.title = ""
        b.isBordered = false
        b.setButtonType(.momentaryChange)
        b.refusesFirstResponder = true      // keep the text selection live
        b.wantsLayer = true

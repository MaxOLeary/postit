...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    private func build(with data: NoteData) {
        ...
        // insert-section — a left-facing triangle that adds a collapsible
        // section (title + foldable body) after the block you're editing.
        let sectionBtn = symbolButton("arrowtriangle.left.fill", point: 11,
                                      action: #selector(insertSection))
        sectionBtn.frame = NSRect(x: 86, y: (Style.stripHeight - 16) / 2, width: 16, height: 16)
        sectionBtn.autoresizingMask = [.maxXMargin, .minYMargin]
        sectionBtn.toolTip = "Add a collapsible section"
    ...
    @objc private func insertSection() {
        ...
        let index = min(activeBlockIndex() + 1, contentStack.arrangedSubviews.count)
        insertBlockView(sv, at: index)
        // Keep a text block after it so you can keep typing below the section.

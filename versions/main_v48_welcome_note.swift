...
private enum Style {
    ...
    static let mathResultColor = NSColor(calibratedRed: 0.55, green: 0.87, blue: 0.56, alpha: 0.95)
...
final class SectionView: NSView, NSTextFieldDelegate {
    ...
    /// selection contract as the body (RR/YY/BB/WW ink, ## new section). The
    ...
    /// fires the action — select a word, type YY, and the word survives with
    /// the title turning yellow, instead of vanishing under the first Y.
...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    func textView(_ textView: NSTextView, shouldChangeTextIn affectedCharRange: NSRange,
        ...
        // of an ink pair (YY at a line start) still fires on the second tap.
        if let s = replacementString, s.count == 1, let ch = s.first,

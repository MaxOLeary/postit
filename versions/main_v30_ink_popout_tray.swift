...
private enum Style {
    ...
    // They live in a popout tray: hover the hollow ring to slide them out,
    // click one to color the selection (or the ink you type with from the
    // cursor on), click the active one again to go back to the default white,
    // and click the ring to tuck the tray away.
    static let inks: [(name: String, color: NSColor)] = [
        ...
        ("Blue",   NSColor(calibratedRed: 0.180, green: 0.302, blue: 0.655, alpha: 1)), // #2E4DA7
    ]
}
...
/// A borderless button that also reports mouse-over — the ink-tray ring uses
/// it so hovering slides the swatches out without needing a click.
...
final class NoteController: NSObject, NSTextViewDelegate, NSWindowDelegate {
    ...
    private var inkRing: HoverButton!
    ...
    private func build(with data: NoteData) {
        ...
        // slide out to its right. Click a swatch to ink the selection / typing
        // color (click it again to deselect), click the ring to close the
        // tray. The ring's center fills with the active ink while collapsed.
        let ringX: CGFloat = 92
        let dotY = (Style.stripHeight - 12) / 2
        inkRing = HoverButton(frame: NSRect(x: ringX, y: dotY, width: 12, height: 12))
        inkRing.title = ""
        ...
        inkRing.onHover = { [weak self] in self?.setInkTray(expanded: true) }
        inkRing.toolTip = "Ink colors — hover to open, click to close"
        ...
        for (i, inkDef) in Style.inks.enumerated() {
            ...
            dot.toolTip = "\(inkDef.name) ink — click again for default"
    ...
    /// Ring click: close the tray (or reopen it — hover normally does that).
    @objc private func inkRingTapped() { setInkTray(expanded: !inkTrayExpanded) }

    /// Swatch click: toggle the ink, recolor any selection, and aim the typing
    /// attributes so the next character comes out in the chosen ink.
    @objc private func inkTapped(_ sender: NSButton) {
        let color = Style.inks[sender.tag].color
        ink = (ink == color) ? nil : color
